...
class Transient:
    ...
    def ext_evt(self, obs):
        def get_src_bkg_idx(self, tt, xx, yy): # For light curves and spectra
            ...
            sr2 = (rad < self.src_rad**2)
            bk2 = (rad > self.bkg_rad[0]**2) & (rad < self.bkg_rad[1]**2)
            return src, bkg, sr2, bk2
        ...
        src, bkg, sr2, bk2 = get_src_bkg_idx(self, obs.t2, obs.xx, obs.yy)
        self.src_tt = obs.t2[src]
        self.src_pi = obs.ee[src]
        self.bkg_tt = obs.t2[bkg]
        self.bkg_pi = obs.ee[bkg]
        self.sr2_tt = obs.t2[sr2]
        self.sr2_pi = obs.ee[sr2]
        self.bk2_tt = obs.t2[bk2]
        self.bk2_pi = obs.ee[bk2]
        ...
        order = np.argsort(self.sr2_tt)
        self.sr2_tt = self.sr2_tt[order]
        self.sr2_pi = self.sr2_pi[order]
        order = np.argsort(self.bk2_tt)
        self.bk2_tt = self.bk2_tt[order]
        self.bk2_pi = self.bk2_pi[order]
    ...
    def mk_lc(self):
        dt2 = (self.tb[-1]-self.tb[0])/lc_nbin
        self.tl2 = np.linspace(self.sr2_tt[0], self.sr2_tt[-1], np.ceil((self.sr2_tt[-1]-self.sr2_tt[0])/dt2).astype('int'))
        self.sr2_lc, _ = np.histogram(self.sr2_tt, self.tl2)
        self.sr2_lc = self.sr2_lc/np.diff(self.tl2)
        self.bk2_lc, _ = np.histogram(self.bk2_tt, self.tl2)
        self.bk2_lc = self.backscal*self.bk2_lc/np.diff(self.tl2)
        self.su2_lc = self.sr2_lc - self.bk2_lc
    ...
    def mk_postcard(self, ctr):
        ...
        tmp = np.empty(2*self.tl2.size-2)
        tmp[:-1:2] = self.tl2[:-1]
        tmp[1::2] = self.tl2[1:]
        shift = int(tmp[0]//1000*1000)
        tmp = tmp - shift
        plt.plot(tmp, self.sr2_lc.repeat(2), label='Source', color='greenyellow')
        plt.plot(tmp, self.bk2_lc.repeat(2), label='Background', color='gold')
        plt.plot(tmp, self.su2_lc.repeat(2), label='Subtracted', color='k')
        ax.set_title(self.out)
        ax.legend()
        ax.set_xlabel('Time-{0:d} (s)'.format(shift))
        ax.set_ylabel('Rate (cts/s)')
        ...
        # Zoomed image
        for ii in range(0,2):
            ax = plt.subplot2grid((3,3), (ii,2), colspan=1, rowspan=1)
...
cwd = '/Users/silver/out/ros_ia/cat/'
ouf = '/Users/silver/out/ros_ia/fig/'
oud = '/Users/silver/out/ros_ia/dat/'
...
dT = [0.005, 0.01] # s
nu_min = 0.01 # Hz
po_min = 18 # Fourier power
tbins = np.logspace(2,4,3) # s
test_stat_lim = 25 # Used 25 for NuSTAR
