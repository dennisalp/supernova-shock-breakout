...
class Xoi:
    def __init__(self, ii):
        ...
        # Parse meta data
        self.target = obs_list['TARGET'][np.argmax(obs_list['OBS_ID'] == obs[ii])]
        buf = 100*'#' + '\nIndex ' + str(ii) + '\n' + 100*'#' + '\n\n'
        buf = buf + 'IAU ID: ' + iauid[ii] + ', Obs. Date: ' + Time(t0[ii], format='mjd').iso + '\n'
        buf = buf + 'Obs. ID: ' + obs[ii] + ', Source number: ' + str(num[ii]) + '({0:03X}'.format(num[ii]) + '), Target: ' + self.target + '\n'
...
#        plt.imshow(gaussian_filter(np.log10(observation.img+1),2).T, vmin=0, vmax=np.percentile(gaussian_filter(np.log10(observation.img+1), 2), 99.9), cmap='afmhot', origin='lower')
    ...
    def mk_postcard(self):
        ...
        out_name = figp
        if self.extremely_flaring:
            out_name += '1_extremely_flaring/'            
        elif self.very_flaring:
            out_name += '2_very_flaring/'
        elif self.flaring:
            out_name += '3_flaring/'
        elif self.agn and self.pval < plim:
            out_name += '4_agn/'
        elif self.persistent and self.pval < plim:
            out_name += '5_persistent/'
        elif self.pval < plim:
            out_name += '6_variable/'
        else:
            out_name += '7_constant/'
...
#        fig.subplots_adjust(hspace = 0.5)
        ...
        plt.close()
        np.savetxt(out_name.replace('.pdf', '.txt'), np.c_[self.src_lc, self.bkg_lc, self.sub_lc])
        return 1
...
################################################################
# Just read the data
figp = '/Volumes/pow/out/sne/sbo/constant/' + sys.argv[1] + '/fig/'
...
cwd = '/Volumes/pow/out/sne/sbo/constant/' + sys.argv[1] + '/dat/'
...
################################################################
retcode = subprocess.call(['mkdir', '-p', figp + '1_extremely_flaring'])
retcode = subprocess.call(['mkdir', '-p', figp + '2_very_flaring'])
retcode = subprocess.call(['mkdir', '-p', figp + '3_flaring'])
retcode = subprocess.call(['mkdir', '-p', figp + '4_agn'])
retcode = subprocess.call(['mkdir', '-p', figp + '5_persistent'])
retcode = subprocess.call(['mkdir', '-p', figp + '6_variable'])
retcode = subprocess.call(['mkdir', '-p', figp + '7_constant'])

processed = set()
for ff in glob('/Volumes/pow/out/sne/sbo/custom/pdf/*.pdf', recursive=True):
    processed.add(ff.split('/')[-1].replace('.pdf',''))
...
ctr1 = 0
ctr2 = 0
for ii, entry in enumerate(dat[start:]):
    # Housekeeping
    ii = ii + start
    ctr1 += 1

    # Skip results from previous runs
    if obs_num.format(obs[ii], num[ii]) in processed:
        continue

#    if np.mod(ii, 1) == 0:
#        print('{0:4d}, {1:5.3f}'.format(ii, ii/dat.size))

    # Collect all objects from the same observation
    # Concerning the latitutde choice of 15 deg, see Fig. 8 of winkel16
    if ext[ii] < 1.e-5 and np.abs(bb[ii]) > 15 and flag_sum[ii] < 3 and det_ml[ii] > det_lim:
        obj = Xoi(ii)
        if obj.keep():
            objects.append(obj)

    # Process the observation
    if len(objects) > 0 and (ii == obs.size-1 or not obs[ii] == obs[ii+1]):
        if download_data(ii):
            observation = Observation(ii)
            
            # Process all objects in the observation
            for xoi in objects:
                xoi.ext_evt(observation.wcs)
                if xoi.src_tt.size <= 3*cts_per_bin:
                    continue
                xoi.mk_img(observation)
                xoi.mk_lc()
                xoi.mk_spec()
                ctr2 += xoi.mk_postcard()
            
            retcode = subprocess.call(['rm', '-rf', cwd + obs[ii]])
            del(observation)
        else:
            print('WARNING No data found for Obs. ID', obs[ii])

        print('Finished processing Obs. ID: {0:10s} (Index: {4:d}), Objects: {1:d}/{2:d}/{3:d}'.format(obs[ii], ctr1, len(objects), ctr2, ii))
        objects = []

    if (ii == obs.size-1 or not obs[ii] == obs[ii+1]):
        ctr1 = 0
        ctr2 = 0
...
#      0  32464  32465
# 160000 196489 196490
# 320000 361358 361359
# 450000 490782 490783
# 610000 646502 646503
