...
buf  = '    \\colhead{SBO} & \\colhead{$t$} & \\colhead{$R$}     & \\colhead{$F_\\mathrm{mean}$}                   & \\colhead{$F_\\mathrm{peak}$}                   & \\colhead{$F_\\mathrm{before}$}                 & \\colhead{$F_\\mathrm{after}$}                  & \\colhead{Rise} & \\colhead{Decay}\\\\\n'
buf += '    \\colhead{}    & \\colhead{(s)} & \\colhead{(\\Rsun)} & \\colhead{($10^{-13}$~erg~s$^{-1}$~cm$^{-2}$)} & \\colhead{($10^{-13}$~erg~s$^{-1}$~cm$^{-2}$)} & \\colhead{($10^{-13}$~erg~s$^{-1}$~cm$^{-2}$)} & \\colhead{($10^{-13}$~erg~s$^{-1}$~cm$^{-2}$)} & \\colhead{}     & \\colhead{}} \\startdata\n'
...
# HR    plt.sca(ax1)
    ...
    def fmt_val(lab):
        ce = vals['cts2erg']/vals['mean'][0]*1.e13
        if lab == 'rise':
            tmp = vals['peak'][0]/vals['before'][0]
            if len(vals['before']) == 1: return '$>{0:.2f}$'.format(tmp)
            else: return'${0:.2f}$'.format(tmp)
        elif lab == 'decay':
            tmp = vals['peak'][0]/vals['after'][0]
            if len(vals['after']) == 1: return '$>{0:.2f}$'.format(tmp)
            else: return'${0:.2f}$'.format(tmp)
        elif lab == 'mean' or lab == 'peak' or ((lab == 'before' or lab == 'after') and len(vals[lab]) == 2):
            return '${0:.3f}\pm{1:.3f}$'.format(ce*vals[lab][0], ce*vals[lab][1])
        else:
            return '$<{0:.3f}$'.format(ce*vals[lab][0])

    rad = int(np.round(cc*np.diff(dur[oi])[0]/(1+vals['zz'])/Rsun))
    buf += '  {0:16s}& {1:13d} & {2:17d} & {3:46s}& {4:46s}& {5:46s}& {6:46s}& {7:15s}& {8:15s}\\\\\n'.format(
        xrt[oi], np.diff(dur[oi])[0], rad, fmt_val('mean'), fmt_val('peak'), fmt_val('before'),
