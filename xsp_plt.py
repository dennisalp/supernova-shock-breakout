...
def get_bf():
    ...
    for line in reversed(list(ff)):
        ...
        elif line[:7] == '     3 ' and 'bb_during_zz_lim' in bfp.keys() and seen:
            bfp['bb_during_zz_low'] = float(line.split()[1])
            bfp['bb_during_zz_upp'] = float(line.split()[2])
        elif line[:7] == '     3 ':
            bfp['bb_during_zz_lim'] = float(line.split()[2])
            bfp['bb_during_zz_li2'] = float(line.split()[1])
...
def fmt_val(lab):
    ...
    if np.abs(low) < 1.e-6 and np.abs(lim) < 1.e-6:
        ...
        return '$<{0:.2f}$'.format(lim)
    elif np.abs(upp) < 1.e-6:
        return '$>{0:.2f}$'.format(li2)
    else:
        return '${0:.2f}_{{{1:.2f}}}^{{+{2:.2f}}}$'.format(val, low-val, upp-val)
...
################################################################
# Parse the MC logs
buf  = '      \\colhead{SBO} & \\colhead{$N_\\mathrm{H,MW}$}     & \\colhead{$z_\\mathrm{f}$} & \\colhead{$N_\\mathrm{H,bb}$}     & \\colhead{$T$}          & \\colhead{$T_1$}        & \\colhead{$T_2$}        & \\colhead{$N_\\mathrm{H,pl}$}     & \\colhead{$\\Gamma$}     & \\colhead{$\\Gamma_1$}   & \\colhead{$\\Gamma_2$}   \\\\\n'
buf += '      \\colhead{}    & \\colhead{($10^{22}$~cm$^{-2}$)} & \\colhead{}               & \\colhead{($10^{22}$~cm$^{-2}$)} & \\colhead{(keV)}        & \\colhead{(keV)}        & \\colhead{(keV)}        & \\colhead{($10^{22}$~cm$^{-2}$)} & \\colhead{}             & \\colhead{}             & \\colhead{}} \\startdata\n'

app  = '      \\colhead{SBO} & \\colhead{$C_\\mathrm{BB}/$dof} & \\colhead{$G_\\mathrm{BB}$} & \\colhead{$C_\\mathrm{BB,1}/$dof} & \\colhead{$G_\\mathrm{BB,1}$} & \\colhead{$C_\\mathrm{BB,2}/$dof} & \\colhead{$G_\\mathrm{BB,2}$} & \\colhead{$C_\\mathrm{PL}/$dof} & \\colhead{$G_\\mathrm{PL}$} & \\colhead{$C_\\mathrm{PL,1}/$dof} & \\colhead{$G_\\mathrm{PL,1}$} & \\colhead{$C_\\mathrm{PL,2}/$dof} & \\colhead{$G_\\mathrm{PL,2}$}} \\startdata\n'
...
for oi in obs[:]:
    ...
    ################################################################
    # Table
    buf += '    {9:16s}& {10:32s}& {0:26s}& {1:23s}& {2:32s}& {3:23s}& {4:32s}& {5:23s}& {6:23s}& {7:23s}& {8:23s}\\\\\n'.format(
        fmt_val('bb_during_zz'), fmt_val('bb_during_nh'), fmt_val('bb_during_tt'), fmt_val('bb_first_tt'), fmt_val('bb_second_tt'),
        fmt_val('pl_during_nh'), fmt_val('pl_during_gg'), fmt_val('pl_first_gg'), fmt_val('pl_second_gg'), xrt[oi], '${0:.3f}$'.format(nh))
    ...
    # plt.gca().axvline(bfp['bb_during_zz_upp'], color='#1f77b4', alpha=0.5)
    ...
    app += '    {0:16s}& {1:29s} & {2:25.2f} & {3:31s} & {4:27.2f} & {5:31s} & {6:27.2f} & {7:29s} & {8:25.2f} & {9:31s} & {10:27.2f} & {11:31s} & {12:27.2f} \\\\\n'.format(
        xrt[oi],
