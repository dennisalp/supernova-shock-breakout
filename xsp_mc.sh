...
@${oi}_set_${2}_${1}.xcm
fit
error 3.5267403802617308 2 4 6
error 12.67311108710059 2 4 6
save model ${oi}_mo_${2}_${1}.xcm
" > ${oi}_fit_${2}_${1}.xcm
    elif [ "${2}" == "bb" ]; then
        printf "@${oi}_load_${1}.xcm
...
@${oi}_set_${2}_${1}.xcm
fit
error 2.295748928898636 4 6
error 10.44286372078092 4 6
save model ${oi}_mo_${2}_${1}.xcm
" > ${oi}_fit_${2}_${1}.xcm        
    fi
...
10.
=p3
=log((1.+190.83804419899818*exp(-0.09003682511287042*p8^2.059933583946007))*0.5087576618016646^7.637255835183834*exp(-0.014369532596800662*p8^(-1.6554116783698598))*3.9984905221170706e+46)-log(${tt}/(1.+p3))
0.3 0.01 0.03 0.03 3. 3.
=p3
1. 0
" > ${oi}_set_bb_during.xcm

# from xsp_frac.py Fraction of energy within 0.3-10. keV
# c1 -190.83804419899818
# aa -0.09003682511287042
# bb -0.014369532596800662
# g1  2.059933583946007
# g2 -1.6554116783698598
# uu  0.5087576618016646
# vv  7.637255835183834
...
# old conditions    xspec < ${oi}_mc_bb_during.xcm > bb_log/${oi}_bb_during_${ii}.log
# old conditions    if [ "$(tail -1 bb_log/${oi}_bb_during_${ii}.log | awk '{print $1;}')" == "C-Statistic" ]  || [ "$(tail -4 bb_log/${oi}_bb_during_${ii}.log | head -1 | awk '{print $1;}')" = "Ill-formed" ]; then
# old conditions        ./fix_seg_fault.sh ${oi} during
# old conditions        xspec < ${oi}_mc_bb_during_post_fix.xcm >> bb_log/${oi}_bb_during_${ii}.log
# old conditions    fi
# old conditions    xspec < ${oi}_mc_bb_first.xcm > bb_log/${oi}_bb_first_${ii}.log
# old conditions    if [ "$(tail -1 bb_log/${oi}_bb_first_${ii}.log | awk '{print $1;}')" == "C-Statistic" ]  || [ "$(tail -4 bb_log/${oi}_bb_first_${ii}.log | head -1 | awk '{print $1;}')" = "Ill-formed" ]; then
# old conditions        ./fix_seg_fault.sh ${oi} first
# old conditions    xspec < ${oi}_mc_bb_first_post_fix.xcm >> bb_log/${oi}_bb_first_${ii}.log
# old conditions    fi
# old conditions    xspec < ${oi}_mc_bb_second.xcm > bb_log/${oi}_bb_second_${ii}.log
# old conditions    if [ "$(tail -1 bb_log/${oi}_bb_second_${ii}.log | awk '{print $1;}')" == "C-Statistic" ]  || [ "$(tail -4 bb_log/${oi}_bb_second_${ii}.log | head -1 | awk '{print $1;}')" = "Ill-formed" ]; then
# old conditions        ./fix_seg_fault.sh ${oi} second
# old conditions    xspec < ${oi}_mc_bb_second_post_fix.xcm >> bb_log/${oi}_bb_second_${ii}.log
# old conditions    fi
# old conditions    rm *.fak
# old conditions
# old conditions    xspec < ${oi}_mc_pl_during.xcm > pl_log/${oi}_pl_during_${ii}.log
# old conditions    if [ "$(tail -1 pl_log/${oi}_pl_during_${ii}.log | awk '{print $1;}')" == "C-Statistic" ]  || [ "$(tail -4 pl_log/${oi}_pl_during_${ii}.log | head -1 | awk '{print $1;}')" = "Ill-formed" ]; then
# old conditions        ./fix_seg_fault.sh ${oi} during
# old conditions        xspec < ${oi}_mc_pl_during_post_fix.xcm >> pl_log/${oi}_pl_during_${ii}.log
# old conditions    fi
# old conditions    xspec < ${oi}_mc_pl_first.xcm > pl_log/${oi}_pl_first_${ii}.log
# old conditions    if [ "$(tail -1 pl_log/${oi}_pl_first_${ii}.log | awk '{print $1;}')" == "C-Statistic" ]  || [ "$(tail -4 pl_log/${oi}_pl_first_${ii}.log | head -1 | awk '{print $1;}')" = "Ill-formed" ]; then
# old conditions        ./fix_seg_fault.sh ${oi} first
# old conditions    xspec < ${oi}_mc_pl_first_post_fix.xcm >> pl_log/${oi}_pl_first_${ii}.log
# old conditions    fi
# old conditions    xspec < ${oi}_mc_pl_second.xcm > pl_log/${oi}_pl_second_${ii}.log
# old conditions    if [ "$(tail -1 pl_log/${oi}_pl_second_${ii}.log | awk '{print $1;}')" == "C-Statistic" ]  || [ "$(tail -4 pl_log/${oi}_pl_second_${ii}.log | head -1 | awk '{print $1;}')" = "Ill-formed" ]; then
# old conditions        ./fix_seg_fault.sh ${oi} second
# old conditions    xspec < ${oi}_mc_pl_second_post_fix.xcm >> pl_log/${oi}_pl_second_${ii}.log
# old conditions    fi
# old conditions    rm *.fak
done
}
