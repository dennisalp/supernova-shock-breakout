    ...
    def find_sources(self):
        ...
        def search(xx, yy, tt):
            ...
            for ii in np.where(p_poisson & enough_counts & high_relative_amp)[0]:
                ...
                if c0 > clim_conv and c0/nn > 0.35:
                    r0 = (xg[i1[0]], yg[i1[1]])
                    t = tt[i0]
                    t0 = t.min()
                    t1 = t.max()
        ...
        for ii in tqdm(range(12)):
            ...
            for kk in range(6):
                ...
                x0 = self.dx[mm].min()
                x1 = self.dx[mm].max()
                dx = int(np.ceil((x1 - x0)/2))

                for nn in range(2):
                    oo = (self.dx >= x0 + nn*dx) & (self.dx <= x0 + (kk+1)*dx)
                    pp = mm & oo
                    if pp.sum() == 0:
                        continue

                    search(xx[pp], yy[pp], tt[pp])
        ...
        if len(self.obj) < 20:
            ...
            for oi, obj in enumerate(self.obj):
                ...
                if src.keep(ctr):
                    ...
                    src.mk_lc()
                    src.mk_spec()
                    if src.bkg_llc[np.argmax(src.tllc > src.t0) - 1] > 0.1:
                        ctr -= 1
                        continue
            ...
            print(f'Skipping creation of {len(self.obj)} sources')
